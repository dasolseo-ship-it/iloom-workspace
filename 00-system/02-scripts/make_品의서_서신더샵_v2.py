...
rf(c5.paragraphs[0].add_run('특별 (세트구매 프로모션)'), bold=True)
for ln in ['지정 세트 구성 시 구매금액의 5% 추가 혜택 (조건 충족 시)']:
...
ct = doc.add_table(rows=9, cols=7); tb(ct)
...
cost_data = [
    ...
    ('입주박람회 경품',      200_000,   100_000, '50%',   100_000, '50%', '부가세 없음 / 예가'),
    ('입주카페 홍보',        200_000,   100_000, '50%',   100_000, '50%', '부가세 없음 / 예가'),
    ('기존혜택\n(목표매출 5%)', 5_000_000, 5_000_000, '100%',       0, '0%',  '1억 × 5% / 100% 본사 부담'),
    ('특별프로모션\n(세트구매 5%)', 1_500_000,  750_000, '50%',  750_000, '50%', '1억 × 30% × 5%\n세트구매비율 30% 가정'),
    ('합계',            10_800_000, 7_900_000, '73%', 2_900_000, '27%', ''),
]
...
hdr_row(sp, ['프로모션명','내용','비고'])
sp_data = [
    ('세트구매\n프로모션',
     '아래 지정 세트 구성 시 구매금액의 5% 추가 혜택\n'
     '  ① 다이닝: 토스카노 + 휴스턴 세트 구매\n'
     '  ② 옷장: 옷장 몸통 3통 이상\n'
     '  ③ 소파: 3인 이상 (1인 블케 포함)',
     '본사:대리점 = 5:5 부담\n상시입주/박람회 프로모션과 중복 가능'),
    ('기간','2026년 8월 박람회 기간 (확정 후 기재)','기간 내 수주 등록건에 한해 적용'),
    ('건명','고객명 뒤 \'(서신)\' 기입\n예시) 홍길동(서신더샵)',''),
]
...
for t in [
    ...
    '- 납기 종료 후 익월말 포인트 일괄 지급 (단, 특별 프로모션(세트구매)은 ERP 등록 없이 영업담당자 별도 정산)',
]: bullet(t)

doc.add_paragraph()
sub('4) 정산')
for t in [
    '- 입주 공략 비용: 2026년 8월 (참가비 / 사인물 / 경품 / 입주카페홍보)',
    '- 프로모션 비용: 2026년 12월 (납기한도: 2026년 11월)',
...
# 저장
OUT = r'20-operations/24-입주공략/전주서신더샵비발디/2026-08_전주서신더샵비발디_입주공략_품의_v4.docx'
doc.save(OUT)
print(f'저장: {OUT}')
