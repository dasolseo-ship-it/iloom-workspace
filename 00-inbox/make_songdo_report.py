...
# ── 차트 색상 범례 (테이블 아래 B~F 영역) ─────
r_cl = r_sum      # 합계행 바로 아래
rh(r_cl, 24)

c_label = ws.cell(row=r_cl, column=2, value="차트 범례")
c_label.font      = fnt(bold=True, size=9, color=C_WHITE)
c_label.fill      = fill(C_SEC_HDR)
c_label.alignment = aln("center", "center")
c_label.border    = bd()

for col, (yr, clr) in enumerate([("2023년","5B9BD5"), ("2024년","70AD47"),
                                  ("2025년","ED7D31"), ("2026년","C00000")], start=3):
    c = ws.cell(row=r_cl, column=col, value=f"  ■  {yr}")
...
r_cl2 = r_sum + 1
rh(r_cl2, 20)
ws.merge_cells(start_row=r_cl2, start_column=2,
               end_row=r_cl2,   end_column=LAST_COL)
c_avg_note = ws.cell(row=r_cl2, column=2,
    value="  ——  검정 실선: 연도별 월 평균 매출  (계단식 — 연도 전환 시 갱신)")
c_avg_note.font      = fnt(size=9, italic=True, color="444444")
c_avg_note.fill      = fill(C_EMPH_BG)
c_avg_note.alignment = aln("left", "center")
c_avg_note.border    = bd()
...
out = r"c:\Users\FURSYS\Downloads\iloom-workspace-claude\00-inbox\송도점_퇴점추진현황_20260610.xlsx"
wb.save(out)
print(f"저장 완료: {out}")
